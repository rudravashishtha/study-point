...
export const createAdminTestAction = withActor(
  withAuthorization(
    "ADMIN",
    withRevalidation(["/admin/tests"], async (actor, input: CreateTestInput) => {
      const res = await createTest(actor.userId, input);
      if (!res.success) {
        throw new DomainError(
          (res.error?.code as DomainErrorCode) || "INTERNAL_ERROR",
          res.error?.message || "Failed to create test",
        );
      }
    ...
    withRevalidation(
      ...
      async (actor, id: string, input: UpdateTestInput) => {
        ...
        if (!res.success) {
          throw new DomainError(
            (res.error?.code as DomainErrorCode) || "INTERNAL_ERROR",
            res.error?.message || "Failed to update test",
          );
        }
        return { success: true, data: res.data };
      },
    ...
    withRevalidation(
      ...
      async (actor, id: string) => {
        ...
        if (!res.success) {
          throw new DomainError(
            (res.error?.code as DomainErrorCode) || "INTERNAL_ERROR",
            res.error?.message || "Failed to publish test",
          );
        }
        return { success: true, data: res.data };
      },
    ...
    withRevalidation(
      ...
      async (actor, id: string) => {
        ...
        if (!res.success) {
          throw new DomainError(
            (res.error?.code as DomainErrorCode) || "INTERNAL_ERROR",
            res.error?.message || "Failed to archive test",
          );
        }
        return { success: true, data: res.data };
      },
