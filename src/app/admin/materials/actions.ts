      ...
      async (actor, input: CreateStudyMaterialInput) => {
        ...
        if (!res.success) {
          throw new DomainError(
            ...
            res.error?.message || "Failed to create material",
          );
        }
    ...
    withRevalidation(
      ...
      async (actor, id: string, input: UpdateStudyMaterialInput) => {
        ...
        if (!res.success) {
          throw new DomainError(
            (res.error?.code as DomainErrorCode) || "INTERNAL_ERROR",
            res.error?.message || "Failed to update material",
          );
        }
        return { success: true, data: res.data };
      },
    ...
    withRevalidation(
      ...
      async (actor, id: string) => {
        ...
        if (!res.success) {
          throw new DomainError(
            (res.error?.code as DomainErrorCode) || "INTERNAL_ERROR",
            res.error?.message || "Failed to publish material",
          );
        }
        return { success: true, data: res.data };
      },
    ...
    withRevalidation(
      ...
      async (actor, id: string) => {
        ...
        if (!res.success) {
          throw new DomainError(
            (res.error?.code as DomainErrorCode) || "INTERNAL_ERROR",
            res.error?.message || "Failed to archive material",
          );
        }
        return { success: true, data: res.data };
      },
