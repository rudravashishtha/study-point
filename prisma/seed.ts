...
async function main() {
  ...
  // Teacher Assignment
  const existingAssignment = await prisma.teacherAssignment.findFirst({ where: { teacherId: teacher.id, batchId: batch.id } });
  ...
  const existingEnrolment = await prisma.enrolment.findFirst({ where: { studentId: student.id, academicSessionId: session.id } });
